...
char *generatePassword(){
    ...
    mvwprintw(menu_win, 6, 5, "Your generated password is %s", genPass);
...
void add_monster (struct ROOM room) {
    int prob;
    if (level == 1)
        prob = difficulty_coeff * 50;
    else if (level == 2)
        prob = difficulty_coeff * 30;
    else if (level == 3)
        prob = difficulty_coeff * 25;
    else
        prob = difficulty_coeff * 20;
...
void reveal_corridor (int px, int py) {
    //int CORRIDOR_VISIBLE = difficulty_coeff * 2 + 1;
    int CORRIDOR_VISIBLE = 2;
...
void calculate_score () {
    score = hits*50*difficulty_coeff + gold*0.5;
}
...
char * food_name (int color) {
    char * name [] = {"Ambrosia", "SlightlyMoldyCheese", "Rock-hardBiscuit", "QuestionableApple", "MysteryMeat", "InfernalSteak", "EtherealBerries", "ExplodingBerries", "Pie"};
    if (color == 5) {
        int which = rand () % 3;
        return name[which];
    } else if (color == 2) {
...
void food_window () {
    ...
    for ( int i = 0; i < food_count; i ++) {
        if (strcmp(foods[i].name, "ExplodingBerries") ==0 && foods[i].state == 0) ex_berry++;
        else if (strcmp(foods[i].name, "EtherealBerries") ==0 && foods[i].state == 0) eth_berry++;
        else if (strcmp(foods[i].name, "Pie") ==0 && foods[i].state == 0) pie++;
        else if (strcmp(foods[i].name, "Ambrosia") ==0 && foods[i].state == 0) amb++;
        else if (strcmp(foods[i].name, "SlightlyMoldyCheese") ==0 && foods[i].state == 0) cheese++;
        else if (strcmp(foods[i].name, "Rock-hardBiscuit") ==0 && foods[i].state == 0) biscuit++;
        else if (strcmp(foods[i].name, "InfernalSteak") ==0 && foods[i].state == 0) steak++;
        else if (strcmp(foods[i].name, "MysteryMeat") ==0 && foods[i].state == 0) meat++;
        else if (strcmp(foods[i].name, "QuestionableApple") ==0 && foods[i].state == 0) apple++;
    }
    ...
    int identifier = 1;
    if (ex_berry != 0) {
        mvwprintw(food, identifier + 1, 1, "%d. %d ExplodingBerries", identifier, ex_berry);
        ex_berry_id = identifier;
        identifier++;
    }
    if (eth_berry != 0) {
        mvwprintw(food, identifier + 1, 1, "%d. %d EtherealBerries", identifier, eth_berry);
    ...
    if (cheese != 0) {
        mvwprintw(food, identifier + 1, 1, "%d. %d SlightlyMoldyCheese",identifier, cheese);
        cheese_id = identifier;
        identifier ++;
    }
    if (biscuit != 0) {
        mvwprintw(food, identifier + 1, 1, "%d. %d Rock-hardBiscuit ",identifier, biscuit);
        biscuit_id = identifier;
        identifier ++;
    }
    if (steak != 0) {
        mvwprintw(food, identifier + 1, 1, "%d. %d InfernalSteak",identifier, steak);
        steak_id = identifier;
        identifier ++;
    }
    if (apple != 0) {
        mvwprintw(food, identifier + 1, 1, "%d. %d QuestionableApple",identifier, apple);
        apple_id = identifier;
        identifier ++;
    }
    if (meat != 0) {
        mvwprintw(food, identifier + 1, 1, "%d. %d MysteryMeat",identifier, meat);
        meat_id = identifier;
        identifier ++;
    }
    ...
    if (choice == ex_berry_id) {
        food_choice("ExplodingBerries");
    }
    else if (choice == eth_berry_id) {
        food_choice("EtherealBerries");
    }
        ...
        food_choice("SlightlyMoldyCheese");
    }
    else if (choice == biscuit_id) {
        food_choice("Rock-hardBiscuit");
    }
    else if (choice == steak_id) {
        food_choice("InfernalSteak");
    }
    else if (choice == meat_id) {
        food_choice("MysteryMeat");
    }
    else if (choice == apple_id) {
        food_choice("QuestionableApple");
    }
...
void save_game (char * name, int px, int py) {
    ...
    fprintf(save_game, "%s ", user_name);
    fprintf(save_game, "%d ", health);
    fprintf(save_game, "%d ", hunger);
    fprintf(save_game, "%d ", hits);
    fprintf(save_game, "%d ", gold);
    fprintf(save_game, "%d ", level);
    fprintf(save_game, "%d ", food_count);
    fprintf(save_game, "%d ", locked_door_count);
    fprintf(save_game, "%d ", secret_door_count);
    fprintf(save_game, "%d ", traps_count);
    fprintf(save_game, "%d ", gold_count);
    fprintf(save_game, "%d ", monster_count);
    fprintf(save_game, "%d ", potion_count);
    fprintf(save_game, "%d %d ", px, py);
    fprintf(save_game, "%d\n", weapon_count);

    for ( int k = 0; k < food_count; k++) {
        fprintf(save_game, "%s %d %d %d %d\n",foods[k].name, foods[k].state, foods[k].color, foods[k].x, foods[k].y );
    }
    
    for ( int k = 0; k < locked_door_count; k ++) {
        fprintf(save_game, "%d %d %d\n",locked[k].state, locked[k].x, locked[k].y );
    }
    
    for ( int k = 0; k < secret_door_count; k ++) {
        fprintf(save_game, "%d %d %d\n",hiddens[k].state, hiddens[k].x, hiddens[k].y );
    }
    
    for ( int k = 0; k < traps_count; k ++) {
        fprintf(save_game, "%d %d %d\n",traps[k].state, traps[k].x, traps[k].y );
    }
    
    for ( int k = 0; k < gold_count; k ++) {
        fprintf(save_game, "%d %d %d %d\n",golds[k].type, golds[k].state, golds[k].x, golds[k].y );
    }

    for ( int k = 0; k < monster_count; k ++) {
        fprintf(save_game, "%c %d %d %d %d %d %d %d\n",monsters[k].type, monsters[k].state, monsters[k].num, monsters[k].health, monsters[k].movement_num, monsters[k].movement_state, monsters[k].x, monsters[k].y );
    }
    
    for ( int n = 0; n < potion_count; n ++) {
        fprintf(save_game, "%d %d %d %d\n", potions[n].state, potions[n].type, potions[n].x, potions[n].y );
    }
    
    for ( int m = 0; m < weapon_count; m ++) {
        fprintf(save_game, "%c %d %d %d %d\n", weapons[m].symbol, weapons[m].num_collect, weapons[m].state, weapons[m].x, weapons[m].y );
    }

    for (int i = 0; i < 6; i++){
        fprintf(save_game, "%d %d %d %d %d\n", rooms[level][i].y, rooms[level][i].x, rooms[level][i].height, rooms[level][i].width, rooms[level][i].type);
    }
...
void load_game(char * file_name) {
    ...
    strcat(arg, ".txt");
    FILE *save_file = fopen(arg, "r");
    if (!save_file) {
        perror("No saved game found.");
        return;
    }

    fscanf(save_file, "%s ", user_name);
    fscanf(save_file, "%d ", &health);
    fscanf(save_file, "%d ", &hunger);
    fscanf(save_file, "%d ", &hits);
    fscanf(save_file, "%d ", &gold);
    fscanf(save_file, "%d ", &level);
    fscanf(save_file, "%d ", &food_count);
    fscanf(save_file, "%d ", &locked_door_count);
    fscanf(save_file, "%d ", &secret_door_count);
    fscanf(save_file, "%d ", &traps_count);
    fscanf(save_file, "%d ", &gold_count);
    fscanf(save_file, "%d ", &monster_count);
    fscanf(save_file, "%d ", &potion_count);
    fscanf(save_file, "%d %d ", &pxx, &pyy);
    fscanf(save_file, "%d\n", &weapon_count);

    for (int k = 0; k < food_count; k++) {
        fscanf(save_file, "%s %d %d %d %d\n", foods[k].name, &foods[k].state, &foods[k].color, &foods[k].x, &foods[k].y);
    }
    ...
    for (int m = 0; m < weapon_count; m++) {
        fscanf(save_file, "%c %d %d %d %d\n", &weapons[m].symbol, &weapons[m].num_collect, &weapons[m].state, &weapons[m].x, &weapons[m].y);
    }

    for (int i = 0; i < 6; i++)
        fscanf(save_file, "%d %d %d %d %d\n", &rooms[level][i].y, &rooms[level][i].x, &rooms[level][i].height, &rooms[level][i].width, &rooms[level][i].type);
...
void generate_map(){
    ...
    while (1) {
        ...
        render_map();
        init_colors();
        if (CharcterColor == 1) attron(COLOR_WHITE);
        else attron(COLOR_PAIR(CharcterColor));
        mvaddch(py, px, '@');
        if (CharcterColor == 1) attroff(COLOR_WHITE);
        else attroff(COLOR_PAIR(CharcterColor));
        refresh();
        ...
        if (ch == KEY_UP || ch == 'J') ny -= speed;
        else if (ch == KEY_DOWN || ch == 'K') ny+= speed;
        else if (ch == KEY_LEFT || ch == 'H') nx-= speed;
        else if (ch == KEY_RIGHT || ch == 'L') nx+= speed;
        else if (ch == 'Y') {
            nx-= speed;
            ny-= speed;
        }
...
void show_profile () {
    ...
    wattron(profile, COLOR_PAIR(CharcterColor));
    ...
    wattroff(profile, COLOR_PAIR(CharcterColor));
...
void get_info(const char* prompt, char* dest, int max_length, int pass, int maybe) {
    int ch, i = 0;
    int rows, cols;
    getmaxyx(stdscr, rows, cols);
    int win_width = 50;
...
void play_menu() {
    ...
    char *options[] = {"Play as a Guest", "Login", "Register", "Forgot Password", "Generate Password", "Exit"};
    int height = 12, width = 40;
    int rows, cols;
    ...
    while (1) {
        ...
        if (ch == KEY_UP && choice > 0) choice--;
            ...
            if (choice == 0) {
                ...
                get_info("Enter username: ", username, 50, 0, 0);
                get_info("Enter password: ", password, 50, 1, 0);
                ...
                while (1) {
                    get_info("Enter password: ", password, 50, 1, 1);
                ...
                while (1) {
                    get_info("Enter email: ", email, 50, 0, 0);
                ...
                get_info("Enter your username: ", username, 50, 0, 0);
                show_pop_up("Do you promise it's you? :D", 0, 0);
